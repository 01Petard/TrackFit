...
const trainingTypeLabels = { strength: '力量', cardio: '慢步', mobility: '拉伸', other: '其他' } as const
      ...
      <div v-else class="hidden overflow-x-auto md:block">
        <table class="w-full text-left text-sm">
          <thead class="border-b border-default bg-elevated/60 text-xs text-muted"><tr><th class="px-5 py-4 font-medium">时间</th><th class="px-5 py-4 font-medium">类型</th><th class="px-5 py-4 font-medium">内容</th><th class="px-5 py-4 font-medium">备注</th><th v-if="store.canWrite.value" class="px-5 py-4 text-right font-medium">操作</th></tr></thead><tbody>
            <tr v-for="item in timeline" :key="`${item.kind}-${item.id}`" class="border-b border-default last:border-0"><td class="whitespace-nowrap px-5 py-4">{{ dayjs(item.occurredAt).format('YYYY-MM-DD HH:mm') }}</td><td class="px-5 py-4"><span class="rounded-lg bg-elevated px-2.5 py-1 text-xs">{{ item.kind === 'training' ? '训练' : '睡眠' }}</span></td><td class="px-5 py-4">{{ item.training ? `${trainingTypeLabels[item.training.type]} · ${item.training.durationMinutes} 分钟` : `就寝 ${dayjs(item.sleep!.fellAsleepAt).format('MM-DD HH:mm')} · 起床 ${dayjs(item.sleep!.wokeUpAt).format('MM-DD HH:mm')} · ${durationLabel(item.sleep!.durationMinutes)} · 分数 ${item.sleep!.quality}` }}</td><td class="max-w-56 truncate px-5 py-4 text-muted">{{ item.training?.note || '—' }}</td><td v-if="store.canWrite.value" class="whitespace-nowrap px-5 py-4 text-right"><button class="mr-3 text-primary" @click="openEdit(item)">编辑</button><button class="text-error" @click="remove(item)">删除</button></td></tr>
          </tbody>
        </table>
      </div>
      <div v-if="timeline.length" class="divide-y divide-default md:hidden">
        <article v-for="item in timeline" :key="`${item.kind}-${item.id}`" class="p-4"><div class="flex items-start justify-between gap-3"><div><span class="text-xs font-medium text-primary">{{ item.kind === 'training' ? '训练' : '睡眠' }}</span><strong class="mt-1 block">{{ item.training ? trainingTypeLabels[item.training.type] : durationLabel(item.sleep!.durationMinutes) }}</strong><p class="mt-1 text-xs text-muted">{{ item.training ? dayjs(item.occurredAt).format('MM月DD日 HH:mm') : `就寝 ${dayjs(item.sleep!.fellAsleepAt).format('MM-DD HH:mm')} · 起床 ${dayjs(item.sleep!.wokeUpAt).format('MM-DD HH:mm')}` }}</p></div><div v-if="store.canWrite.value" class="flex gap-3 text-sm"><button class="text-primary" @click="openEdit(item)">编辑</button><button class="text-error" @click="remove(item)">删除</button></div></div><p class="mt-3 text-xs text-muted">{{ item.training ? `${item.training.durationMinutes} 分钟` : `睡眠分数 ${item.sleep!.quality}` }}<span v-if="item.training?.note"> · {{ item.training.note }}</span></p></article>
      </div>
    </section>
