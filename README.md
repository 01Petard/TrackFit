...
访问 `http://127.0.0.1:3000`。开发服务监听 `0.0.0.0`，同一局域网内也可以通过 `http://<本机局域网 IP>:3000` 访问。
...
当前数据格式版本为 v3。读取 v1/v2 备份时会自动转换训练强度和睡眠质量字段，并在下一次保存时写成 v3，不需要 SQL 或手工改数据。
