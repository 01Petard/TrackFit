...
test('桌面端和手机端均可维护训练与睡眠记录', async ({ page }, testInfo) => {
  ...
  await dialog.getByLabel('主观强度（1–10）').fill('8')
  ...
  await page.getByRole('button', { name: '＋ 睡眠' }).click()
  await dialog.getByLabel('睡眠质量（百分制）').fill('80')
  await dialog.getByRole('button', { name: '保存记录' }).click()
  const sleepRecord = page.locator('tr:visible, article:visible').filter({ hasText: '质量 80%' }).filter({ has: page.getByRole('button', { name: '编辑' }) }).first()
  await expect(sleepRecord).toBeVisible()
  await sleepRecord.getByRole('button', { name: '编辑' }).click()
  await dialog.getByLabel('睡眠质量（百分制）').fill('90')
  await dialog.getByRole('button', { name: '保存记录' }).click()
  const updatedSleep = page.locator('tr:visible, article:visible').filter({ hasText: '质量 90%' }).filter({ has: page.getByRole('button', { name: '删除' }) }).first()
